...
- If a tool failed only because a precondition is missing that any available tool can establish (a required target location/resource does not yet exist, or an OS-level prerequisite is absent), do NOT give up. Select the corrective action — this can be the SAME tool with different args (e.g. re-run vm.run_command with `mkdir -p <dir> && <original_command>`, prepend a missing `cd` parent, install a missing package via the same shell tool) OR a different tool that establishes the precondition. Re-attempt the original action on the next step. Escalate to ask_user or done only after a self-repair attempt has itself failed.
- Mechanically-fixable shell stderr patterns ALWAYS take a corrective step before failing: `No such file or directory`, `cd: ... can't cd to`, `mkdir: ...: File exists` (already satisfied — proceed), `command not found` for a tool installable via the same shell, `EACCES`/`Permission denied` on a writable parent, `ENOENT`. For "no such directory" specifically: emit `mkdir -p <missing_dir> && <original_command>` as the next vm.run_command and continue. Do NOT ask the user to create the directory.
...
- Before returning status="failed" for a precondition the agent itself can fix (a target location/resource that an available tool can create, or a prerequisite that an available tool can satisfy — including the SAME tool with corrective args like `mkdir -p`, `cd ..`, installing via the same shell), prefer status="continue" so the next step runs the corrective action and then re-attempts the original. Reserve status="failed" for failures no available tool can repair: a disabled module/permission/toggle, an unavailable capability, or a genuinely unrecoverable error.
- For shell tools (vm.run_command and similar): stderr containing `No such file or directory`, `cd: can't cd to`, `ENOENT`, or `command not found` (when the missing command is installable via the same shell) is mechanically fixable. Return status="continue" and let the next step issue `mkdir -p <dir> && <retry>` or the equivalent install. Do NOT return status="failed" on the first occurrence of these patterns.''';
...
- The ORIGINAL USER REQUEST is the only goal that decides done. Compare the work actually performed against that request — not against subgoal labels alone. If the user asked for X (e.g. "build a landing page and serve it") and the only successful tool so far did Y (created a directory, installed a CLI, fetched a path, ran vm.status), the user's goal is NOT met. Return status="continue".
