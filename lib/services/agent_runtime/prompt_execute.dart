...
- App Agentic screen automation loop:
  ...
  * READING CONTENT FOR SUMMARY/EXTRACTION (generic — applies to ANY app: social feeds, articles, chats, comments, emails):
    - When the goal is to read/summarize/extract content from items on screen (posts, articles, messages, etc.), the content you summarize MUST come from the actual on-screen text, not a guess. Read the node text/desc fields of the target item.
    - EXPAND TRUNCATED TEXT FIRST: many apps clip long content and show an inline expand affordance — a small clickable node whose text/desc means "see more / read more / show more / continue / …" (in ANY language, e.g. an ellipsis "…", "lainnya", "selengkapnya", "more", "ver más", "もっと見る"). If a target item's text looks cut off AND a sibling/child node offers such an expand control, app_agent.click that control FIRST, then app_agent.inspect again, and summarize from the now-full text. Do NOT summarize from the truncated preview.
    - DO NOT SKIP MEDIA ITEMS: a post/item that is image/video/link-based (little or no body text) still counts as content. Describe it from what IS available — author/handle, caption/alt text (often in the node `desc`/content-description), and the fact that it contains an image/video/link. Treat it as one of the requested items; never silently skip it just because it lacks a long text body.
    - COUNT REQUESTED ITEMS HONESTLY: if the user asked for N items, collect N distinct items (scrolling + inspecting as needed). If after reasonable scrolling fewer than N exist, summarize what genuinely exists and say so — do not pad or fabricate.
    - ACCUMULATE ACROSS SCREENS, don't wait for all N at once: items do NOT need to be visible on a single screen. Read each item as you encounter it and remember it in your reasoning; once you have read item 1's full content, scroll to find item 2, and so on. After you have read the full content of all N items (across however many screens), proceed to summarize. Do NOT keep scrolling/re-inspecting hoping to see all N simultaneously — that loops forever on feeds that show one item per viewport.
    - PREFER A CONCRETE ACTION OVER RE-SCROLLING: if the latest inspect shows an unexpanded "see more" affordance on the current item, the next action is app_agent.click on THAT node — not another scroll. Only scroll once the current visible item is fully read.
    - DEGRADE GRACEFULLY IF EXPANSION FAILS: if you click an expand control and the next app_agent.inspect shows the item's text is unchanged (still clipped), try at most once more. If it still does not expand, STOP clicking that control and summarize the item from the visible text you already have, noting the content may be partial. A failed expand control must NEVER block the whole task — a partial summary of the readable text is the correct outcome, not repeated clicks or giving up.
...
- CONTENT COMPLETENESS (generic, any app — read/summarize/extract goals):
  * Do NOT accept status="done" for a summary/extraction built from CLIPPED text when an inline expand affordance ("see more / read more / …" in any language) was visible and NEVER clicked. Return status="continue" so the selector expands it first, then re-inspects.
  * EXCEPTION — attempted-and-failed expansion: if previous_results show the expand control was already clicked one or more times AND a later inspect shows the text stayed clipped (the expand did not take effect), DO accept status="done" with a partial summary built from the visible text. A failed expand is NOT a reason to keep looping or to fail the task.
  * Do NOT accept a result that SKIPPED media/image/video/link items the user asked to include. Those are valid items — they should be represented by author + caption/desc + media type. If they were dropped, return status="continue".
  * If the user asked for N items and fewer distinct items were actually read, only accept done when the screen genuinely has fewer (after scrolling) — otherwise continue collecting.
