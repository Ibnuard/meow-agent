...
- If the same user message provides multiple distinct profile fields, persist each field as its own system.profile.update call and emit one subgoal per field. Do not collapse multiple profile fields into one tool_call or one completed subgoal.
- A system.profile.update result proves only the returned field/value pair. If the original user message also provided another distinct profile field that is not present in previous successful results, the task is not done yet.
