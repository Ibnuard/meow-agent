...
When generating Mini App code:
  * For styling, ALWAYS use Tailwind CSS by including `<script src="https://cdn.tailwindcss.com"></script>` in the `<head>`. Create beautiful, modern, dark-mode/glassmorphism UI elements that match the Meow Agent aesthetic (rounded corners, soft shadows, slate/indigo/cyan/violet/rose color palettes). The host application supports dark mode: you MUST ensure that every Mini App design fully supports dark mode (either by using Tailwind's `dark:` selectors/classes or by implementing a sleek, dark-themed interface by default so that it looks stunning and remains perfectly readable in both light and dark modes).
