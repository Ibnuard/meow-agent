...
void main() {
  ...
  test('S21 app_agent task with App Agentic off fails before opening any app',
    ...
    // Isolated in-memory ledger + unique agent: the preflight now PARKS a
    // resumable ledger (so "lanjut" can restart), which must not bleed into
    // other scenarios that share the default on-disk ledger DB.
    final ledgerDb = TaskLedgerDatabase(overrideDbPath: inMemoryDatabasePath);
    final res = await buildEngine(llm: llm, router: router, ledgerDb: ledgerDb)
        .run(
      req('buka facebook cari 2 post di timeline', agentId: 'a1_s21'),
      provider: provider(),
    );
  ...
  // ── Scenario 22: restart-on-resume for app-automation ──────────────────
  // A multi-step app task ("open FB, read 2 posts") was parked when a
  // permission was off (pendingTool device.foreground_app). The user grants
  // the permission and says "lanjut". The engine must RESTART from the original
  // goal (re-analyze → re-open FB), NOT replay the stale parked tool against
  // now-changed foreground state (user left FB to flip the toggle). Asserts the
  // analyze phase runs and the parked device.foreground_app is NEVER dispatched.
  test('S22 app-automation resume restarts from original goal, not stale tool',
      () async {
    final ledgerDb = TaskLedgerDatabase(overrideDbPath: inMemoryDatabasePath);
    // Seed a parked multi-step app-automation ledger (pending tool is the
    // stale foreground read; >1 subgoal → planRequiresAppAgent → restart).
    final parked = TaskLedger(
      id: 'lg_parked',
      agentId: 'a1_s22',
      source: LedgerSource.chat,
      mainGoal: 'open facebook and read 2 posts',
      languageCode: 'id',
      originalUserMessage: 'buka facebook lalu cek 2 postingan di timeline',
      goalTree: GoalTree(
        mainGoal: 'open facebook and read 2 posts',
        subgoals: [
          Subgoal(id: 'sg1', label: 'open facebook'),
          Subgoal(id: 'sg2', label: 'read 2 posts'),
          Subgoal(id: 'sg3', label: 'summarize'),
        ],
      ),
      pendingToolName: 'device.foreground_app',
      pendingToolArgs: const {},
    );
    await ledgerDb.upsert(parked);

    // Scripted phases keyed off the ORIGINAL message being re-analyzed: a
    // fresh plan that re-opens FB. If the engine instead replayed the parked
    // tool, app.resolve would never be dispatched.
    final llm = ScriptedLlmClient({
      'analyze': [
        '{"intent":"open.fb","goal":"open facebook read 2 posts",'
            '"requires_tools":true,"risk":"safe","tool_groups":["app","app_agent"],'
            '"missing_info":[],"subgoal_seeds":["open facebook","read 2 posts",'
            '"summarize"],"task_relation":"continuation","narrative":""}',
      ],
      'reflect': [
        '{"strategy":"direct_execute","goal_tree":{"main_goal":"open fb",'
            '"completion_criteria":["fb open"],"subgoals":[{"id":"sg1",'
            '"label":"open facebook","required_slots":{},"missing_slots":[],'
            '"status":"pending"}]},"narrative":""}',
      ],
      'plan': [
        '{"main_goal":"open fb","completion_criteria":["fb open"],'
            '"subgoals":[{"id":"sg1","label":"open facebook","required_slots":{},'
            '"missing_slots":[],"status":"pending"}],"narrative":""}',
      ],
      'selectTool': [
      ...
      'review': [
        '{"status":"done","final_response":"Opened.",'
            '"subgoal_update":{"id":"sg1","status":"done"},"narrative":""}',
      ],
      'verbalize.success': ['Done.'],
      'verbalize.answer_from_tool_result': ['Facebook is open.'],
    });
    final router = ScriptedToolRouter(
      results: {
        'app.resolve': const ToolExecutionResult(
      ...
      // App Agentic now GRANTED (user enabled it) — nothing denied.
    );

    await buildEngine(llm: llm, router: router, ledgerDb: ledgerDb)
        .run(req('sudah saya izinkan, lanjutkan', agentId: 'a1_s22'),
            provider: provider());

    // The stale parked tool must NEVER fire.
    expect(router.dispatchCountOf('device.foreground_app'), 0);
    // The task was re-analyzed from the original goal (analyze ran) and the
    // app was re-opened via the fresh plan (app.resolve dispatched).
