...
void main() {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R3 no capability — does not fabricate success',
    () async {
      if (!EnvLoader.isAvailable) return;
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R4 ambiguous request — asks clarifying question',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R5 multi-target — create three notes',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
      ...
      final res = await engine.run(
        req(
          'create three notes: one about meeting agenda, one about groceries, '
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R6 language detection — replies in Indonesian',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R7 sensitive tool — parks for confirmation',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
      ...
      final res = await engine.run(req('open WhatsApp'), provider: provider());
      ...
      expect(
        res.state,
        anyOf(
          AgentRuntimeState.waitingConfirmation,
          AgentRuntimeState.done,
          AgentRuntimeState.askingUser,
          AgentRuntimeState.failed,
        ),
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R8 list agents — returns list without hallucination',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R9 empty search — honest empty, no retry',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R10 device info — chained safe reads',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R11 cross-reference — create then verify',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R12 conversational — no tools, direct response',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(results: const {});
      final engine = buildEngine(router: router);
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R13 failed tool — reports failure honestly',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R14 multi-subgoal + confirmation — create agent then delete',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
          ...
          'agent.delete': const ToolExecutionResult(
            ...
            data: {'deleted': true, 'name': 'TestBot', 'id': 'a2'},
          ),
        },
      );
      final engine = buildEngine(router: router);
      ...
      if (res.state == AgentRuntimeState.done) {
        expect(router.dispatchCountOf('agent.delete'), greaterThanOrEqualTo(1));
      }
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R15 bulk predicate — delete notes with pattern in title',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R16 complex chained — create, read, update, delete agent',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
          ...
          'agent.delete': const ToolExecutionResult(
            success: true,
            toolName: 'agent.delete',
            data: {'deleted': true, 'name': 'LinkBot', 'id': 'chain_test'},
          ),
        },
      );
      final engine = buildEngine(router: router);
      ...
      final res = await engine.run(
        req(
          'create an agent called ChainBot, then rename it to LinkBot, '
          'then delete it',
        ),
        provider: provider(),
      );
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R17 Indonesian multi-target — compound request in ID',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R18 empty ecosystem — honest about no data',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
      ...
      expect(
        router.dispatchSequence.where(
          (t) =>
              t.startsWith('system.agents.') ||
              t.startsWith('system.workflows.') ||
              t.startsWith('system.modules.'),
        ),
        isEmpty,
      );
    },
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R19 failed tool — recovery attempts, honest outcome',
    () async {
      if (!EnvLoader.isAvailable) return;
      final router = ScriptedToolRouter(
        results: {
      ...
      final res = await engine.run(
        req(
          'create a note called "System Design Notes" with details about '
  ...
  // ═══════════════════════════════════════════════════════════════════════
  test(
    'R20 conversational greeting — direct response, no tools',
    () async {
      if (!EnvLoader.isAvailable) return;
